 ...
 */
{
  "playersPanel": {
    ...
    "large": {
      ...
      "nickFormatLeft": "<b><font color='{{c:rating}}'>{{rating%2d~%}}</font></b>  <font color='{{c:xwn8}}'>{{xwn8}}</font> {{nick}}",
      "nickFormatRight": "{{nick}} <font color='{{c:xwn8}}'>{{xwn8}}</font>  <b><font color='{{c:rating}}'>{{rating%2d~%}}</font></b>",
      ...
      "vehicleFormatLeft": "{{vehicle%-16s}} <b><font color='{{c:hp-ratio}}'>{{hp-ratio%2d~%}}</font></b>",
      "vehicleFormatRight": "{{vehicle%-16s}} <b><font color='{{c:hp-ratio}}'>{{hp-ratio%2d~%}}</font></b>",
      // Display format for frags (macros allowed, see readme-en.txt).
      // Формат отображения фрагов (допускаются макроподстановки, см. readme-ru.txt).
      "fragsFormatLeft": "{{frags}}",
      "fragsFormatRight": "{{frags}}",
